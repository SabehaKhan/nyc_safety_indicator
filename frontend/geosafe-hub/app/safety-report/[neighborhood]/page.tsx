...
export default function SafetyReportPage() {
  ...
  const { neighborhood: slug } = useParams(); //dynamic segment
  ...
  useEffect(() => {
    if (slug && typeof slug === 'string') {
      const titleCaseNeighborhood = getTitleCaseNeighborhood(slug);
      if (titleCaseNeighborhood) {
        setCurrentNeighborhood(titleCaseNeighborhood);
        fetchSafetyScore(titleCaseNeighborhood);
        // ... fetch other data based on neighborhood
      } else {
        console.error("Invalid neighborhood slug:", slug);
        setCurrentNeighborhood(null);
        setNotFound(true);
      }
    }
  }, [slug, fetchSafetyScore]); // Empty dependency array ensures this runs only once on mount

  const boroname = currentNeighborhood
    ? nycLocations.boroughs.includes(currentNeighborhood)
      ? currentNeighborhood
      : getBoronameFromNtaname(currentNeighborhood)
    : null;
          ...
          <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-xl p-8 shadow-xl mb-8">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
              <div>
                ...
                {currentNeighborhood ? `${currentNeighborhood} Safety Report` : 'Loading Report...'}
                </h1>
              ...
              <>
                <div className="relative pt-1 mb-6">
                  <div className="overflow-hidden h-2 rounded bg-gray-700">
                    <div style={{ width: `${safetyScore}%` }} className="bg-blue-500 h-full" />
                  </div>
                ...
                <div className="mb-8">
                  ...
                  <ReviewList ntaname={currentNeighborhood} boroname={boroname || ""} />
                </div>
              </>
            )}
            {notFound && <p className="text-white">Neighborhood not found.</p>}
            {!currentNeighborhood && !notFound && <p className="text-white">Loading safety report...</p>}
          </div>
        </div>
      </main>
